...
CREATE TABLE `stateprovince` (
  ...
  `CountryRegionCode` varchar(8) DEFAULT NULL,
  `IsOnlyStateProvinceFlag` tinyint(1) DEFAULT NULL,
  `Name` varchar(250) DEFAULT NULL,
  `TerritoryId` int(10) DEFAULT NULL,
  PRIMARY KEY (`StateProvinceId`),
  UNIQUE KEY `Name_UNIQUE` (`Name`),
  UNIQUE KEY `code` (`StateProvinceCode`,`CountryRegionCode`),
  KEY `CountryRegionc` (`CountryRegionCode`),
  KEY `TerritoryIDe` (`TerritoryId`),
  CONSTRAINT `CountryRegionc` FOREIGN KEY (`CountryRegionCode`) REFERENCES `countryregion` (`CountryRegionCode`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `TerritoryIDe` FOREIGN KEY (`TerritoryId`) REFERENCES `salesterritory` (`TerritoryId`) ON DELETE NO ACTION ON UPDATE NO ACTION
