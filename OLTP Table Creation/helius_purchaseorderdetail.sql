...
CREATE TABLE `purchaseorderdetail` (
  ...
  `LineTotal` decimal(19,4) DEFAULT NULL,
  `ReceivedQty` decimal(8,2) DEFAULT NULL,
  `RejectedQty` decimal(8,2) DEFAULT NULL,
  `StockedQty` decimal(9,2) DEFAULT NULL,
  PRIMARY KEY (`PurchaseOrderDetailId`,`PurchaseOrderId`),
  KEY `Proddcct` (`ProductId`),
  KEY `PurchaseOrder` (`PurchaseOrderId`),
  CONSTRAINT `Proddcct` FOREIGN KEY (`ProductId`) REFERENCES `product` (`ProductId`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `PurchaseOrder` FOREIGN KEY (`PurchaseOrderId`) REFERENCES `purchaseorderheader` (`PurchaseOrderId`) ON DELETE NO ACTION ON UPDATE NO ACTION
