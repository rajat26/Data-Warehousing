...
CREATE TABLE `currencyrate` (
  ...
  `CurrencyRateDate` datetime DEFAULT NULL,
  `FromCurrencyCode` char(3) DEFAULT NULL,
  `ToCurrencyCode` char(3) DEFAULT NULL,
  `AverageRate` decimal(19,4) DEFAULT NULL,
  `EndOfDayRate` decimal(19,4) DEFAULT NULL,
  PRIMARY KEY (`CurrencyRateId`),
  UNIQUE KEY `U!` (`CurrencyRateDate`,`FromCurrencyCode`,`ToCurrencyCode`),
  KEY `fromcurrency` (`FromCurrencyCode`),
  KEY `tocurrency` (`ToCurrencyCode`),
  CONSTRAINT `fromcurrency` FOREIGN KEY (`FromCurrencyCode`) REFERENCES `currency` (`CurrencyCode`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `tocurrency` FOREIGN KEY (`ToCurrencyCode`) REFERENCES `currency` (`CurrencyCode`) ON DELETE NO ACTION ON UPDATE NO ACTION
